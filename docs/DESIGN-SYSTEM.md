...
- **Metric strip** — **full-bleed**, edge to edge, `width:100vw` with the container padded
  ...
  at `--t-mega` with `white-space:nowrap`; "226/226" is the constraint that sets the ceiling.
...
is unavailable — those visitors get a text statement pointing at the same 28 agents below.
...
28 agent nodes on the surface, joined by synapse lines under 0.85 units apart, with five
...
and a callout cycling all 28 every 3.6s.
...
1. **Change numbers in both files.** Search `28`, `226`, and the date `2026-09-01`.
